...
export function VideoTile({ participant, isLarge = false, isPinned = false, onPin }: VideoTileProps) {
  return (
    <div
      className={`relative overflow-hidden rounded-xl bg-[#16162e] group transition-shadow duration-200 ${
        participant.isSpeaking
          ? "ring-[2.5px] ring-green-400/70 shadow-[0_0_12px_rgba(74,222,128,0.15)]"
    ...
    >
      {participant.isVideoOn ? (
        ...
        <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-[#1c1c38] to-[#12122a]">
          <div
            className={`rounded-full flex items-center justify-center text-white/80 ${
              isLarge
                ? "w-24 h-24 text-4xl bg-indigo-500/20"
                : "w-14 h-14 sm:w-16 sm:h-16 text-xl sm:text-2xl bg-indigo-500/20"
            }`}
          >
      ...
      <div className="absolute bottom-2 left-2.5 flex items-center gap-1.5">
        <div className="flex items-center gap-1.5 bg-black/30 backdrop-blur-[6px] rounded-md px-2 py-0.5">
          {participant.isMuted ? (
          ...
          <span className="text-white/90 text-xs">
            ...
            {participant.isHost && (
              <span className="ml-1 text-[10px] text-amber-300/80">Host</span>
            )}
          </span>
        </div>
      </div>
