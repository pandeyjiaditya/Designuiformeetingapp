import { X, Mic, MicOff, VideoOff, MoreHorizontal, Search, UserPlus, Crown } from "lucide-react";
  ...
  return (
    <div className="flex flex-col h-full bg-[#13132b] text-white">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.06]">
        <h3 className="text-sm text-white/80">People ({participants.length})</h3>
        <button
          onClick={onClose}
      ...
      <div className="px-3 py-2.5 space-y-1.5">
        <div className="flex items-center gap-2 bg-white/[0.04] rounded-lg px-3 py-2 border border-white/[0.06] focus-within:border-indigo-500/30 transition-colors">
        ...
        <button className="flex items-center gap-2 w-full px-3 py-2 rounded-lg hover:bg-white/[0.04] transition-colors text-indigo-400 text-sm">
      ...
      <div className="flex-1 overflow-y-auto px-2">
        <div className="px-2 py-1.5">
          <span className="text-[10px] text-white/30 uppercase tracking-widest">In this call</span>
        </div>
        {participants.map((p) => (
          <div
            key={p.id}
            className="flex items-center gap-3 px-2 py-2 rounded-lg hover:bg-white/[0.03] group transition-colors"
          >
            <div className="relative flex-shrink-0">
              ...
              />
              {p.isSpeaking && (
                <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-green-400 rounded-full border-2 border-[#13132b]" />
              )}
            </div>
